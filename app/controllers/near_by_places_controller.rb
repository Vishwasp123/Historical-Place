class NearByPlacesController < ApplicationController
  ...
  def index
    @near_by_places = NearByPlace.includes(:place, :tourist_point ,images_attachments: :blob).all
  end
  ...
  def create
    ...
    if @near_by_place.save
      redirect_to near_by_places_path, notice:"near_by_place created!"
    else
      render :new
    end
  end
  ...
  def update
    if @near_by_place.update(near_by_place_params)
      redirect_to near_by_places_path, notice:"near_by_place updated!"
    else
      render :edit
    end
  end
